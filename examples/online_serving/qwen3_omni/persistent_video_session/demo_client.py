...
QUERY = (
    "In one sentence each: (a) what is happening right now, and (b) what was "
    "shown at the very BEGINNING of the video?"
)
...
async def main(a) -> int:
    ...
    print(f"[demo] model={model} frames={n} queries~={n_q} refresh_at={a.refresh_at} "
          f"sink={a.sink} recent={a.recent}", flush=True)

    answers, errors, got_done = [], [], False

    async with websockets.connect(uri, max_size=64 * 1024 * 1024) as ws:
        config = {
            "type": "session.config", "model": model, "modalities": ["text"],
            "persistent": True, "sink_frames": a.sink, "num_frames": a.recent,
            "refresh_at_position": a.refresh_at, "enable_frame_filter": False,
        }
        if a.brief:
    ...
    print("Now confirm in the SERVER log: `[streaming-kv] eviction ... alive=N` stays flat and "
          "new `vsess-...-<epoch>` ids appear (one per refresh).", flush=True)
