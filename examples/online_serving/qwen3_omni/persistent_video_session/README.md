...
**Measured (2×H200, v0.26.0 stack + rebase overlay, CUDA-graphs config):**
...
**Raise `--max-model-len` and `--limit-mm-per-prompt` with it.** The request's token count binds ~10.6× before its positions do (~0.095 positions per token), so size both to the session horizon — and size them to the **measured live rate**, not the processor floor. On a live-paced feed (e.g. 2 fps) the driver's opportunistic 2-frame batching loses its `get_nowait` race against the per-frame reader, so chunks stay **single-frame**: measured cost ≈ **~450–500 tokens / ~48 positions per second of 2 fps video** (~225–250 tokens / ~24 positions per frame at ~640px; confirmed over a 2 h 12 m soak and the 4-stream wave). The processor figure of ~243 tokens / ~23 positions per 2-frame chunk is the **theoretical floor** — reached only when frames arrive back-to-back (flood feeds); a driver-side chunk-batching fix to restore it on live feeds is a known follow-up. Budgeting with the live rate: at the default `--max-model-len 65536` the session is length-capped near position ~6,200 — only ~2 min of live 2 fps video, long before the first rebase — so size to the intended horizon, e.g. `--max-model-len 1048576` ≈ ~35 min of live 2 fps video (`VLLM_ALLOW_LONG_MAX_MODEL_LEN=1` is already exported by `run_server.sh`; KV memory stays bounded by `--streaming-kv-*`, the added cost is host-side buffers; the linear token count itself is the remaining "unbounded" wall — a documented follow-up). The video-item limit counts **chunks** — one per frame on a live feed: reaching the first rebase alone takes ~2,050 chunks (set the limit ≥ ~2,200), a 1M-token horizon holds `1048576/243 ≈ 4300` (rule of thumb: `max-model-len/240`, e.g. `'{"video": 5120}'`) — and **any** rebase-mode run past ~250 live frames needs a raise from the shipped `'{"video": 256}'` (refresh mode never hits it: each epoch re-seeds after ~50 items).
...
| `--max-num-seqs` | server | concurrent persistent sessions per server; the demo default `1` keeps single-viewer latency, `8` validated with 8 concurrent refresh-mode streams on 2×H200 (each stream keeps its own flat 672–673 KV band); engine-rebase sessions validated at `4` concurrent live-paced streams (independent rebases + per-stream opening recall) |
