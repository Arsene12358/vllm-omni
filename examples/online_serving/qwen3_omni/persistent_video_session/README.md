...
Measured on 2×H200 (eager): ingestion ~25 frames/s (**~12× a live 2 fps feed**), query
latency ~1.5 s, KV `alive` 672–673.
...
- **vLLM with the streaming-KV eviction branch.** The eviction (`--streaming-kv-*`) that
  bounds the cache, plus a per-chunk `max_tokens` scheduler fix the persistent mode needs,
  live on [`Arsene12358/vllm@feat/streaming-kv-eviction`](https://github.com/Arsene12358/vllm/tree/feat/streaming-kv-eviction).
  The changes are pure-Python over the `v0.20.0` release, so the quickest path is to install
  the wheel and overlay the branch's changed files:
  ```bash
  pip install vllm==0.20.0
  git clone -b feat/streaming-kv-eviction https://github.com/Arsene12358/vllm.git vllm-fork
  SITE=$(python -c "import vllm,os; print(os.path.dirname(vllm.__file__))")
  ( cd vllm-fork && git fetch --tags >/dev/null 2>&1
    for f in $(git diff --name-only v0.20.0..HEAD -- 'vllm/**/*.py'); do
      rel=${f#vllm/}; mkdir -p "$SITE/$(dirname "$rel")"; cp "$f" "$SITE/$rel"; done )
  ```
- **This vLLM-Omni checkout** (with the persistent handler) installed.
...
- **Queries return one word / stop immediately** — the vLLM-core `max_tokens` scheduler
  fix isn't applied; re-check the eviction-branch overlay above.
...
| `assets/showcase.html` | narrated walkthrough with the embedded recording |
| `assets/recording.mp4` / `.gif` | the captured ~45 s segment |
